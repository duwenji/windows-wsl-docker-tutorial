...
⚠️ この2つはセットで使う必要があります。`-i`だけだとキー入力は送れますが、`bash`は自分が端末に繋がっていると認識できずプロンプト（`$ `など）が表示されない状態になります。逆に`-t`だけだと、端末らしい見た目にはなりますが、標準入力が閉じているためキー入力そのものが届きません。両方揃って初めて、手元のターミナルで打った文字がコンテナ内のシェルに届き、かつシェル側もそれを「対話端末からの入力」として正しく扱える状態になります。
