...
💡 `--driver`を指定していませんが、`docker network create`は既定で`bridge`ドライバを使うため、これは`docker network create --driver bridge sample-net`と同じ意味です。単一ホスト内でコンテナ間通信させたいだけなら、通常は明示的に指定せず既定のまま使えば十分です（他に、ホストのネットワークスタックを共有する`host`、複数ホストにまたがる通信用の`overlay`などのドライバがあります）。
