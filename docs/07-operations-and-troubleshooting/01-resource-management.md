...
💡 `wsl --shutdown`自体は終了コマンドであり、設定を読み込み直す処理は含まれていません。VMを一度壊し、次回起動時にゼロから作り直させることで新しい設定が反映される、という仕組みの詳細は [02-environment-setup/01-install-wsl.md](../02-environment-setup/01-install-wsl.md) を参照してください。
